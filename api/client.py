...
class GranolaAPIClient:
    ...
    def __init__(self, token_manager):
        ...
        self._mcp_session_id = None  # Mcp-Session-Id from server
        self._request_id = 0
    ...
    def _mcp_call(self, method: str, params: Optional[dict] = None, timeout: int = 120) -> dict:
        """Send a JSON-RPC request to the MCP server and return the result.

        Uses httpx streaming to properly handle SSE responses — the server
        may keep the stream open after sending data, so we must read events
        ...
        logger.debug(f"MCP request: method={method}, id={msg['id']}")

        with self._http.stream("POST", MCP_URL, json=msg, headers=headers) as response:
            response.raise_for_status()

            # Capture MCP session ID from server
            ...
            logger.debug(f"MCP {method}: status={response.status_code}, content-type={content_type}")

            if 'text/event-stream' in content_type:
                # Parse SSE events incrementally as they arrive
                for line in response.iter_lines():
                    if line.startswith('data: '):
                        ...
                        if 'result' in data:
                            return data['result']
                raise Exception(f"SSE stream ended without result for {method}")

            # Non-SSE JSON response — read the full body
            body = response.read()
            if not body:
                raise Exception(f"Empty response for {method} (status {response.status_code})")
    ...
    def _mcp_notify(self, method: str, params: Optional[dict] = None):
        ...
        Uses streaming to avoid blocking on the 202 Accepted response —
        the server sends chunked encoding with no body, and a non-streaming
        read would block until the connection is dropped (~60s).
        """
        msg = {'jsonrpc': '2.0', 'method': method}
        if params is not None:
            msg['params'] = params
        with self._http.stream("POST", MCP_URL, json=msg, headers=self._get_headers()) as response:
            logger.debug(f"MCP notify {method}: status={response.status_code}")
            # 202 Accepted — no body to read, close immediately
    ...
    def _ensure_initialized(self):
        ...
        self._mcp_call('initialize', {
            ...
            'capabilities': {},
            'clientInfo': {'name': 'granola-export-gui', 'version': __version__}
        })
    ...
    # Maximum days per chunk for custom date range queries.
    # Granola's MCP server has a ~60-second processing timeout;
    # keeping chunks small avoids hitting it.
    _CHUNK_DAYS = 14

    def _list_meetings_chunked(self, start_date: str, end_date: str) -> List[Dict]:
        """Fetch meetings in weekly-ish chunks to avoid server-side timeouts."""
        start = datetime.fromisoformat(start_date)
        end = datetime.fromisoformat(end_date)
        all_meetings: Dict[str, Dict] = {}  # deduplicate by meeting id
        ...
        while chunk_start <= end:
            ...
            args = {
                'time_range': 'custom',
                'custom_start': chunk_start.strftime('%Y-%m-%d'),
                'custom_end': chunk_end.strftime('%Y-%m-%d'),
            }
            logger.info(f"Fetching meetings chunk: {args['custom_start']} to {args['custom_end']}")
            text = self._call_tool('list_meetings', args)
            for m in self._parse_meetings_xml(text):
                all_meetings[m['id']] = m
            chunk_start = chunk_end + timedelta(days=1)

        meetings = sorted(all_meetings.values(), key=lambda m: m.get('date', ''), reverse=True)
        return meetings
    ...
    def list_meetings(self, date_range: str = "last_30_days",
        ...
        if start_date and end_date:
            logger.info(f"Fetching meetings via MCP: custom {start_date} to {end_date}")
            return self._list_meetings_chunked(start_date, end_date)
            ...
            if custom_start and custom_end:
                logger.info(f"Fetching meetings via MCP: {date_range} -> {custom_start} to {custom_end}")
                return self._list_meetings_chunked(custom_start, custom_end)
            else:
